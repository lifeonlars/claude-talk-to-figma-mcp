...
export function registerModificationTools(server: McpServer): void {
  console.log('Starting to register modification tools...');

  // Set Fill Color Tool
  console.log('Registering set_fill_color tool...');
  server.tool(
    'set_fill_color',
      ...
      try {
        ...
        return {
          content: [
            {
              type: 'text',
              text: `Error setting auto layout: ${
                error instanceof Error ? error.message : String(error)
              }`,
            },
          ],
      ...
      try {
        ...
        return {
          content: [
            {
              type: 'text',
              text: `Error setting effect style: ${
                error instanceof Error ? error.message : String(error)
              }`,
            },
          ],
  ...
  console.log('Registering set_fill_style_id tool...');
  server.tool(
    'set_fill_style_id',
    'Apply a fill/color style to a node in Figma',
    {
      nodeId: z.string().describe('The ID of the node to modify'),
  ...
  console.log('Finished registering modification tools');
}
